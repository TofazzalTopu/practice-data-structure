 ...
 class GFG {
    ...
    public static void main(String[] args) {
        ...
        personList.stream()
                .map(x -> x.getName())
                .forEach(System.out::println);
        personList.stream().forEach(x-> GFG.printPerson(x, "name", 10));
    }

    public static void printPerson(Person p, String name, int age) {
        System.out.println(p.getName() + " " + p.getAge() + " " + name + " " + age);
    }
    @FunctionalInterface
    ...
    public static void applyMethodReference() {
        TriConsumer<Person, String, Integer> triConsumer = GFG::printPerson;
        List<Person> personList = new ArrayList<>();
        personList.stream().forEach(x-> triConsumer.accept(x, "name", 10));
    }
